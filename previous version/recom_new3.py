...
class Act_Rec:
    ...
    def __init__(self):
        ...
        res_gen = [
        ResponseSchema(name="S1", description="탐구를 위한 에세이 주제. 질문이 아닌 주제를 출력해."),
        ...
        ResponseSchema(name="Q2", description="두번째 가이드 질문"),
        ResponseSchema(name="Q3", description="세번째 가이드 질문")]
        self.output_parser_book = StructuredOutputParser.from_response_schemas(res_gen)
        self.output_parser_gen = StructuredOutputParser.from_response_schemas(res_book)
        self.format_instructions_gen = self.output_parser_gen.get_format_instructions()
        self.format_instructions_book = self.output_parser_book.get_format_instructions()
    def general_recommend(self, major):
        ...
        prompt = ChatPromptTemplate(
            ...
            partial_variables={"format_instructions": self.format_instructions_gen} # 출력되는 포맷 입력
            )
